class etherpad_lite (
  $db_name = 'etherpad-lite', $db_user = 'etherpad-lite', $db_host = '127.0.0.1', $db_password = 'absent',
  $nodejs_version = 'installed', $settings_content = 'absent', $settings_source = 'absent',
  $repo = 'git://github.com/Pita/etherpad-lite.git', $repo_ensure = 'present', $repo_rev = 'master',
  $ensure = 'present', $clear_pads = 'absent', $api_url = 'absent', $api_key = 'absent'
) {
...
# This needs changing or commenting out - uses different DB  
  ...
  if $ensure == 'present' {
    mysql_grant{"${db_user}@${db_host}/${db_name}":
      privileges => [ 'alter_priv', 'create_priv', 'select_priv', 'insert_priv',
                      'update_priv', 'delete_priv', 'trigger_priv' ],
  ...
  file {
    ...
    "/srv/etherpad-lite/settings.json":
      ...
      mode => 0640, owner => etherpad-lite, group => etherpad-lite,
      notify => Service["etherpad-lite"],
...
# This needs customizing for Byzantium
  ...
  augeas {
    "logrotate_etherpad":
      ...
      changes => [ 'set file /var/log/etherpad-lite/*.log', 'set rotate 5', 'set schedule daily',
                   'set compress compress', 'set delaycompress delaycompress', 'set ifempty notifempty',
                   'set copytruncate copytruncate', 'set create/mode 0640', 'set create/owner etherpad-lite',
  ...
  if $clear_pads != 'absent' {
    ...
    file {
      "/usr/local/bin/clear-old-pads.rb":
        ...
        mode => 0755, owner => etherpad-lite, group => etherpad-lite;
    }

    cron { "pad_ecology":
      command => "/usr/local/bin/clear-old-pads.rb 2>/dev/null",
      user => etherpad-lite,
      hour => "$clear_pads", minute => 0;
    }
  }
}
